...
<article class="group flex flex-col overflow-hidden rounded-box border border-neutral-200/70 bg-white shadow-sm transition-colors hover:border-neutral-300 dark:border-white/10 dark:bg-neutral-900 dark:hover:border-white/20">
    <a href="{{ route('customer.merchants.show', $menu->merchant) }}" class="relative flex h-44 items-center justify-center border-b border-neutral-100 bg-neutral-100/60 dark:border-white/5 dark:bg-white/5">
        ...
        @endif
        <span class="absolute bottom-3 left-3 rounded border border-neutral-200 bg-white/90 px-2 py-0.5 font-mono text-[10px] font-medium uppercase tracking-wider text-neutral-600 dark:border-white/10 dark:bg-neutral-950/90 dark:text-neutral-300">
            {{ $menu->category }}
        </span>
    </a>
    ...
    <div class="flex flex-1 flex-col justify-between space-y-3 p-4">
        ...
        <div class="flex items-center justify-between border-t border-neutral-100 pt-2.5 dark:border-white/5">
            <span class="font-mono text-sm font-semibold tabular-nums text-neutral-900 dark:text-white">Rp {{ number_format($menu->price, 0, ',', '.') }}</span>
