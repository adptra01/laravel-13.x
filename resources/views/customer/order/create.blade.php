    ...
    <div class="mx-auto max-w-3xl">
        <nav class="mb-4 text-sm text-neutral-500" aria-label="Breadcrumb">
            <ol class="flex items-center gap-1.5">
                <li><a href="{{ route('customer.dashboard') }}" class="transition-colors hover:text-neutral-900">Beranda</a></li>
                <li aria-hidden="true"><x-ui.icon name="ps:caret-right" class="size-3.5" /></li>
                <li><a href="{{ route('customer.merchants.show', $menu->merchant) }}" class="transition-colors hover:text-neutral-900">{{ $menu->merchant->company_name }}</a></li>
                <li aria-hidden="true"><x-ui.icon name="ps:caret-right" class="size-3.5" /></li>
                <li class="font-medium text-neutral-900">Konfirmasi</li>
            </ol>
        </nav>
        ...
        <div class="overflow-hidden rounded-box border border-neutral-200/70 bg-white shadow-sm">
            <div class="border-b border-neutral-100 px-6 py-5">
                <p class="font-mono text-[11px] font-medium uppercase tracking-widest text-zinc-500 dark:text-zinc-400"># Buat pesanan</p>
                <h1 class="mt-1.5 text-xl font-semibold tracking-tight text-neutral-900">Konfirmasi Pesanan</h1>
                <p class="mt-1 text-sm text-neutral-500">Lengkapi detail pesanan di bawah ini.</p>
            </div>

            <div class="p-6">
                {{-- Order summary card --}}
                <div class="flex items-center gap-4 rounded-lg border border-neutral-200/70 bg-neutral-50/60 p-4">
                    ...
                    <div class="min-w-0 flex-1">
                        <p class="text-sm font-semibold tracking-tight text-neutral-900">{{ $menu->name }}</p>
                        <p class="mt-0.5 text-sm text-neutral-500">{{ $menu->merchant->company_name }} · {{ $menu->category }}</p>
                        <p class="mt-1 font-mono text-base font-semibold tabular-nums text-neutral-900">Rp {{ number_format($menu->price, 0, ',', '.') }} / porsi</p>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('customer.orders.store', $menu) }}" class="mt-6 space-y-5">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <p class="mt-1 font-mono text-[11px] text-neutral-400">Stok tersedia: {{ $menu->stock }} porsi</p>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3"
                            class="mt-1.5 w-full rounded-box border border-neutral-200 bg-white px-3.5 py-2.5 text-sm text-neutral-900 placeholder:text-neutral-400 transition-colors focus:border-neutral-900 focus:outline-none"
                    ...
                    {{-- Summary --}}
                    <div class="rounded-lg border border-neutral-200/70 bg-neutral-50/60 p-4 text-sm">
                        <div class="flex items-center justify-between text-neutral-600">
                            <span>Subtotal</span>
                            <span class="multiply-price font-mono text-lg font-semibold tabular-nums text-neutral-900" data-price="{{ $menu->price }}">Rp {{ number_format($menu->price, 0, ',', '.') }}</span>
                        </div>
                        <p class="mt-1 text-xs text-neutral-400">Harga belum termasuk ongkir &amp; PPN. Detail invoice diterbitkan setelah pesanan dikonfirmasi merchant.</p>
                    </div>
