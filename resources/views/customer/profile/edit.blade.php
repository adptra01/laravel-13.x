...
>
    <div class="mx-auto space-y-6">
        ...
        <header class="flex items-center gap-4 rounded-box border border-neutral-200/70 bg-white dark:bg-neutral-900 dark:border-white/10 p-6 shadow-sm">
            ...
            @endif
            <div>
                <p class="text-[11px] font-medium uppercase tracking-widest text-zinc-500 dark:text-zinc-400"># Profil kantor</p>
                <h1 class="mt-1.5 text-xl font-semibold tracking-tight text-neutral-900 dark:text-white sm:text-2xl">{{ $customer->company_name }}</h1>
                <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Informasi ini dipakai untuk alamat pengiriman pesanan Anda.</p>
            </div>
        </header>
            ...
            <form method="POST" action="{{ route('customer.profile.update') }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="mt-1.5 w-full rounded-box border border-neutral-200 bg-white dark:bg-neutral-900 dark:border-white/10 px-3.5 py-2.5 text-sm text-neutral-900 dark:text-white placeholder:text-neutral-400 transition-colors focus:border-neutral-900 dark:focus:border-white focus:outline-none">{{ old('description', $customer->description) }}</textarea>
                ...
                <div>
                    ...
                    <input type="file" id="logo" name="logo" accept="image/*"
                        class="mt-1 block w-full text-sm text-neutral-600 dark:text-neutral-400 file:mr-4 file:rounded-lg file:border-0 file:bg-neutral-100 file:px-4 file:py-2 file:text-sm file:font-medium file:text-neutral-700 dark:text-neutral-300 hover:file:bg-neutral-200" />
