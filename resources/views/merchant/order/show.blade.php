<x-layouts.merchant title="Detail Pesanan">
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="overflow-hidden rounded-box border border-neutral-200/70 bg-white shadow-sm dark:border-white/10 dark:bg-neutral-900">
            ...
            <dl class="divide-y divide-neutral-100 px-5 dark:divide-white/5">
                ...
                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-2">
                    ...
                    <dd class="font-mono text-sm text-neutral-700 tabular-nums dark:text-neutral-300">{{ $order->customer?->phone ?? '-' }}</dd>
                </div>
                <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-2">
            ...
            <x-ui.table>
                ...
                <tfoot>
                    <tr class="border-t border-neutral-200/70 dark:border-white/10">
                        ...
                        <td class="px-5 py-3 text-right font-mono text-sm font-semibold tabular-nums text-neutral-900 dark:text-white">Rp {{ number_format($order->total_price, 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
            </x-ui.table>
        </div>
                ...
                <div class="space-y-2 px-5 py-4">
                    @forelse ($order->payments as $payment)
                        <div class="flex items-center justify-between gap-3 text-sm">
                            <span class="font-mono text-xs text-neutral-500">{{ $payment->transaction_id }}</span>
            ...
            <div class="overflow-hidden rounded-box border border-neutral-200/70 bg-white shadow-sm dark:border-white/10 dark:bg-neutral-900">
                ...
                <div class="space-y-2 px-5 py-4">
                    @forelse ($order->invoices as $invoice)
                        <div class="flex items-center justify-between gap-3 text-sm">
                            <a href="{{ route('merchant.invoices.show', $invoice) }}" class="font-mono text-xs text-neutral-600 transition-colors hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-white">{{ $invoice->invoice_number }}</a>
                            <span class="font-mono text-xs tabular-nums text-neutral-500">Rp {{ number_format($invoice->total, 0, ',', '.') }}</span>
                        </div>
                    @empty
        ...
        @if ($order->status !== 'cancelled' && $order->status !== 'completed')
            <div class="overflow-hidden rounded-box border border-neutral-200/70 bg-white shadow-sm dark:border-white/10 dark:bg-neutral-900">
                <div class="border-b border-neutral-100 px-5 py-3.5 dark:border-white/5">
                    <h2 class="text-sm font-semibold tracking-tight text-neutral-900 dark:text-white">Perbarui Status</h2>
                </div>
                <form method="POST" action="{{ route('merchant.orders.status', $order) }}" class="flex flex-wrap items-center gap-2 px-5 py-4">
                    @csrf
                    @method('PATCH')
                    <x-ui.button type="submit" name="status" value="confirmed" size="sm" variant="outline">Tandai Dikonfirmasi</x-ui.button>
                    <x-ui.button type="submit" name="status" value="cooking" size="sm" variant="outline">Tandai Dimasak</x-ui.button>
                    <x-ui.button type="submit" name="status" value="delivered" size="sm" variant="outline">Tandai Diantar</x-ui.button>
                    <x-ui.button type="submit" name="status" value="completed" size="sm" icon="ps:check">Tandai Selesai</x-ui.button>
                </form>
            </div>
        @endif
    </div>
